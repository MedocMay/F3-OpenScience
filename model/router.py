...
def _openai_style(base, messages, model, key):
    """OpenAI 兼容(OpenAI / DeepSeek / Moonshot / DashScope-compat / 本地 vLLM 等共用)。"""
    hdr = {"authorization": f"Bearer {key}"} if key else {}
    r = _post(base.rstrip("/") + "/chat/completions", hdr, {"model": model, "messages": messages})
...
PROVIDERS = {
    "anthropic": lambda m, model, k: _anthropic(m, model, k or os.environ.get("ANTHROPIC_API_KEY", "")),
    "openai":    lambda m, model, k: _openai_style("https://api.openai.com/v1", m, model, k or os.environ.get("OPENAI_API_KEY", "")),
    "deepseek":  lambda m, model, k: _openai_style("https://api.deepseek.com/v1", m, model, k or os.environ.get("DEEPSEEK_API_KEY", "")),
    "moonshot":  lambda m, model, k: _openai_style("https://api.moonshot.cn/v1", m, model, k or os.environ.get("MOONSHOT_API_KEY", "")),   # Kimi
    "dashscope": lambda m, model, k: _openai_style("https://dashscope.aliyuncs.com/compatible-mode/v1", m, model, k or os.environ.get("DASHSCOPE_API_KEY", "")),  # Qwen
    "gemini":    lambda m, model, k: _gemini(m, model, k or os.environ.get("GEMINI_API_KEY", "")),
    "ollama":    lambda m, model, k: _ollama(os.environ.get("OLLAMA_HOST", "http://localhost:11434"), m, model),
    "openai_compat": lambda m, model, k: _openai_style(os.environ.get("LOCAL_LLM_BASE", "http://localhost:8000/v1"), m, model, k or os.environ.get("LOCAL_LLM_KEY", "")),
}
...
class ModelRouter:
    ...
    def complete(self, messages: list[dict], model: str | None = None, api_key: str | None = None) -> dict:
